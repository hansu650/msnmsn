...
| `code/scripts/monitor_full.ps1` | read-only monitor | Report runner state, exact completion/failure count, current file/arm, GPU, disk, and log freshness at 15-minute intervals for the main arm, ablations, or conditional confirmation seeds. Progress is enumerated from the frozen 530-series manifest, so stale or extra run directories cannot inflate counts; confirmation mode includes only seeds 2028/2029. |
