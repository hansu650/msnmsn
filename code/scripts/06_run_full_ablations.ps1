...
$LogRoot = Join-Path $RepoRoot 'logs\full_ablations'
...
$trajectories = @('PAPERNEG', 'OFFICIAL')
$seed = 2027
$jobIndex = 0
foreach ($trajectory in $trajectories) {
    foreach ($row in $rows) {
        ...
        if (Test-Path -LiteralPath $successPath -PathType Leaf) {
            Write-Output (
                "SKIP_VALID_ABLATION {0}/1060 trajectory={1} track={2} series={3}" -f
                $jobIndex, $trajectory, $row.track, $seriesId
            )
            continue
        }
        $stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
        $logPath = Join-Path $LogRoot (
            "{0:D4}_{1}_{2}_{3}.log" -f $jobIndex, $trajectory, $seriesId, $stamp
        )
        Write-Output (
            "RUN_ABLATION {0}/1060 trajectory={1} track={2} series={3}" -f
            $jobIndex, $trajectory, $row.track, $seriesId
        )
        & $Python @(
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Full ablation job failed with exit code $LASTEXITCODE. Log: $logPath"
        }
        if (-not (Test-Path -LiteralPath $successPath -PathType Leaf)) {
            throw "Full ablation runner did not commit trajectory success: $runDirectory"
        }
    }
}

Write-Output 'FULL_ABLATION_RUNS_COMPLETE completed=1060 failed=0 trajectories=PAPERNEG,OFFICIAL checkpoint=LAST'
